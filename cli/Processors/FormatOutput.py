            ...
            if chunk.type == 'group':
                ...
                if chunk.options['code_echo']:
                    ...
                    if 'prompt' in chunk.options:
                        input = chunk.options['prompt'].format(
                            chunk.reply['content']['execution_count']) + input
                    parts.append(
                        self.format(
                            chunk,
            ...
            for mimetype in chunk.options['mimetypes']:
                if mimetype in msg['content']['data']:
                    ...
                    if chunk.options['math_split'] and mimetype in (
                            'text/latex', 'text/x-latex', 'text/tex', 'text/x-tex'):
                        parts = re.split(
                            r'(?s)(?P<d>[$]{1,2})(.*?)(?P=d)', content)
                        output = ''
                        for i in range(len(parts)):
                            sub = i % 3
                            if sub == 0:
                                output += self.format(chunk,
                                                      'text/latex', None, parts[i])
                            elif sub == 2:
                                output += self.format(chunk,
                                                      'text/x.latex-math',
                                                      None,
